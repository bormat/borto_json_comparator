...
function compare(articlesNode, articlesRuby) {
    ...
    articlesNode.forEach(function(aNode){
        ...
        if(["5ee34c5fbeca882d7edaaa63"].includes(id)) {
            console.error("why ruby does not match this one?")
            return false;
            // console.log(aNode);
        }
        if("5ee34c7bbeca882d7edaab62" == id) {
            console.error("why ruby does not match this one? but node have weird hit they should be merged")
            return false;
        }
        if(["5ee34c7bbeca882d7edaab67"].includes(id)) {
            console.log("Ruby is wrong to not match this one there is \"GE\' in the article");
            return false;
        }
    ...
    articlesRuby.forEach(function(aRuby){
        const id = aRuby._id;
        if(["5ee34c5bbeca882d7edaaa3a"].includes(id)) {
            console.log("Ruby is wrong: why aluminium matches, it should only match if there is Alba");
            return false;
        }
        if(["5ee34c61beca882d7edaaa76"].includes(id)) {
            console.log("Ruby is wrong \"or\" here is not a word to match but an operator");
            return false;
        }
    ...
    Object.entries(relevancyRuby).forEach(function([key, value]){
        if(!relevancyNode[key]){
            ...
            console.log("the keyword ", key, " is not in node", value);
        }
    })
    ...
    Object.entries(relevancyNode).forEach(function([key, value]){
        if(!relevancyRuby[key]){
            ...
            console.log("the keyword ", key, " is not in ruby, article_id", value);
        }
    })
    ...
    // compare the ruby and node article
    for(let arti of artiNodeRuby){
        ...
        if(arti.ruby != arti.node){
            console.error("difference between node and ruby");
        }
        if(arti.ruby.length > arti.node.length){
            ++iij;
    ...
    console.log(toCheck);
    console.log("this array contain the ruby and node article for the same id", artiNodeRuby);
    console.log("finished");
}

let strNode = fs.readFileSync("./mb_regex_tagged_node");
let strRuby = fs.readFileSync("./mb_regex_tagged");
const articlesNode = eval("strNode = " + strNode);
const articlesRuby = eval("strRuby = " + strRuby);
